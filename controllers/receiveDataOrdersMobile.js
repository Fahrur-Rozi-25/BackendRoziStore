...
export const DataOrdersMobile = async (req , res) => {
    ...
    const {transactionID , buyer_sku_code , id , verify ,statusPembayaran , buyerName ,validation} = req.body;
    ...
    if (validation === 'FahrurRoziAPIMobileAPP') {
        console.log("OKK");
        const Order = await OrderDigiflazz(transactionID , buyer_sku_code , id, verify , statusPembayaran , buyerName)
